...
let filter = filters['medium'];
let originaldata = [];
let boxes = [];
let node_clicked = null;
let word_selected = null;
let weights_stats = [];
let filtered_sites = [];
...
function preparingWords(site) {
	...
	return site.data.map((box, index) => {
		...
		const unwanted_words = 'a ante antes bajo con contra de desde después despues' +
		' en es entre hacia hasta para por según sin sobre tras mediante durante el'  +
		...
		// Calculate Weights
		// FontSize
		const fweight = (box['font-size'].replace('px', '') * 1) / 24;
		// Location on the screen
		const lweight = getLocationWeight(site.abr, box.y);
		// Tag Tree
		const tweight = getTagsWeight(box['parent-tags']);
		// Final Weight
...
function getLocationWeight(site, y) {
    ...
    if (y < site_specs[site].top_filter) {return 0; }
    if (y < page_height) {return 1; }
    if (y < page_height * 2) {return 0.5; }
    if (y < page_height * 3) {return 0.2; }
    if (y < page_height * 4) {return 0.1; }
	return 0;
}
...
function countingWords(boxes) {
    ...
    for (let i = 0; i < boxes.length; i++) {
      for (let j = 0; j < boxes[i].words.length; j++) {
        if (wordCount[boxes[i].words[j]]) {
          wordCount[boxes[i].words[j]].weight += boxes[i].weight;
        } else {
          wordCount[boxes[i].words[j]] = {
    ...
    const sorted_sums = Object.keys(wordCount).sort(function(a, b){return wordCount[b].weight - wordCount[a].weight; });
	const return_array = [];
	const weight_threshold = 0;
	for (let j = 0; j < sorted_sums.length; j++) {
		if (wordCount[sorted_sums[j]].weight >= weight_threshold) {
			return_array.push({'word': sorted_sums[j], 'weight': wordCount[sorted_sums[j]].weight});
		}
    }			
	return return_array;
}
...
function connectWords(boxes) {
	...
	for (let i = 0; i < boxes.length; i++) {
      for (let j = 0; j < boxes[i].words.length; j++) {
		  for (let k = j + 1; k < boxes[i].words.length; k++) {
			  const wordkey = (boxes[i].words[j] >= boxes[i].words[k]) ? boxes[i].words[j]+'__'+boxes[i].words[k] : boxes[i].words[k]+'__'+boxes[i].words[j];
			  if (connections[wordkey]) {
				  ...
				  connections[wordkey] = {
					  weight: 1,
					  source: boxes[i].words[j],
					  target: boxes[i].words[k]
				  };
			  }
		  }
      }
...
function loadVisualization() {			
	...
	const words_array = countingWords(boxes);
	const words_connection = connectWords(boxes);
	const nodes = generateNodes(words_array);
	const edges = generateEdges(words_connection);
    ...
    s.bind('clickNode', function(e) {
		...
		s.graph.edges().forEach(function(e) {
			if (toKeep[e.source] && toKeep[e.target])
				e.color = 'rgba(200, 0, 0, 0.2)';
			else
				e.color = 'rgba(100, 100, 100, 0.1)';
		});
...
function reloadVisualization() {		
	...
	jsonresult = JSON.parse(JSON.stringify(originaldata));
...
function loadSitesButtons() {
	...
	$('.sites-button').click(function() {
		let site_selected = $(this).data('filter').toString();
